...
import { PresentationsDeleteRequest } from "../../types/api";
...
export const presentationsDelete = onCall(async (request) => {
    try {
        const ctx = await getAuthContext(request);
        requireAdmin(ctx);

        const { id } = request.data as PresentationsDeleteRequest;
