...
exports.teaPartiesUpdate = void 0;
...
exports.teaPartiesUpdate = (0, https_1.onCall)(async (request) => {
    try {
        const ctx = await (0, auth_1.getAuthContext)(request);
        (0, guard_1.requireAdmin)(ctx);
        const { id, teaParty } = request.data;
        if (!id || !teaParty)
            throw new errors_1.ApiError("invalidArgument", "id と teaParty は必須です");
        const ref = firestore_2.db.collection("teaParties").doc(id);
        const snap = await ref.get();
        if (!snap.exists)
            throw new errors_1.ApiError("notFound", "指定されたお茶会が存在しません");
        await ref.update({ ...teaParty, updatedAt: firestore_1.FieldValue.serverTimestamp() });
        return { ok: true };
    }
    catch (e) {
